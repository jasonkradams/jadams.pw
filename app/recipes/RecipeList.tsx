...
function buildTagUrl(activeTags: string[], clickedTag: string): string {
  const next = activeTags.includes(clickedTag)
    ? activeTags.filter(t => t !== clickedTag)
    : [...activeTags, clickedTag];

  if (next.length === 0) return '/recipes';
    ...
    <div>
      {activeTags.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-8">
          ...
          {activeTags.map(tag => (
            <Link key={tag} href={buildTagUrl(activeTags, tag)}>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-2 mb-4">
                {(recipe.tags || []).map((tag, tagIndex) => (
                  <Link key={tagIndex} href={buildTagUrl(activeTags, tag)}>
                    <Badge
                      variant="secondary"
